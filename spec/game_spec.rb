module TicTacToe
  RSpec.describe Game do
    describe '#start' do
      ...
      subject { game.start }
      it "starts by setting action_to_perform to ask_p1_name" do
        expect{ subject }.to change{game.aasm.current_state}.from(:idle).to(:accepting_p1_name)
        action_to_perform = game.action_to_perform
        expect(action_to_perform.name).to eq(:ask_p1_name)
      end
      ...
      describe '#accept_p1_name' do
        ...
        context "When valid p1 name is provided" do
          ...
          it "Accepts p1 name, transitions to accepting_p2_name state and action_to_perform is ask_p2_name" do
            expect{ subject }.to change{ game.aasm.current_state }.from(:accepting_p1_name).to(:accepting_p2_name)

            expect(game.p1_name).to eq('p1 name')
            action_to_perform = game.action_to_perform
            expect(action_to_perform.name).to eq(:ask_p2_name)
          end
        end
        ...
        context "invalid p1 name is provided" do
          ...
          it "Does not accept the name and remains in accepting_p1_name state" do
            ...
            expect(game.aasm.current_state).to eq(:accepting_p1_name)
            action_to_perform = game.action_to_perform
            expect(action_to_perform.name).to eq(:ask_p1_name)
      ...
      describe '#accept_p2_name' do
        ...
        context "When valid p2 name is provided" do
          ...
          it "transitions to waiting_p1_to_move state and action_to_perform is get_p1_move" do
            expect{ subject }.to change{ game.aasm.current_state }.from(:accepting_p2_name).to(:waiting_p1_to_move)
        ...
        context "When p2 name matches p1 name" do
          ...
          it "Does not accept the name and remains in accepting_p2_name state" do
            ...
            expect(game.aasm.current_state).to eq(:accepting_p2_name)
            action_to_perform = game.action_to_perform
            expect(action_to_perform.name).to eq(:ask_p2_name)
